 ...
 *
 * Interface labels live in src/i18n/{en,ru,uz}.ts; `L()` below is kept as a
 * thin backwards-compatible wrapper around i18n `translate()`.
 */
...
const FINDING_NAMES: Record<string, Tr> = {
  ...
  // ─── BRAIN TRIAGE (validated normal-vs-abnormal) ────────────────────────
  abnormal:           { ru: 'Отклонение (триаж)',     uz: 'Og‘ish (triaj)',  en: 'Abnormal (triage)' },
...
const URGENCY_KEYS: Record<FindingUrgency, I18nKey> = {
  review: 'urgency.review',
  unvalidated: 'urgency.unvalidated',
  not_flagged: 'urgency.notFlagged',
};

export function urgencyWord(urgency: FindingUrgency, lang: Lang = 'ru'): string {
  return translate(lang, URGENCY_KEYS[urgency]);
}
...
export function statusWord(status: string, lang: Lang = 'ru'): string {
  return translate(lang, STATUS_KEYS[status] || STATUS_KEYS.experimental);
}

/**
 * @deprecated Backwards-compatible wrapper — new code should use `t()` / `useT()`
 * from src/i18n. Kept so older call sites keep compiling.
 */
export function L(key: I18nKey, lang: Lang = 'ru'): string {
